# 研究：境内双通道 + 境外跨境支付（多通道统一抽象）

- **查询**: 多通道支付架构（微信/支付宝/连连/PingPong）统一抽象；境内支付宝分账；境外连连/PingPong 跨境收单 vs 收款/结汇 可生产规则；押金多通道兼容；合规前置清单
- **范围**: external（支付宝/连连/PingPong 官方规则，结合领域知识）+ mixed（cool-admin/Midway/TypeORM 落点）
- **日期**: 2026-07-07
...
> **建议**：先与业务方确认「境外」指 A 还是 B（PRD 未决问题）。**默认按 B**（跨境收款/结汇）落地，因为：①跨境电商真实痛点 ②合规门槛低于收单 ③不涉及境外持卡人风控 ④PingPong/连连成熟方案。**A（跨境收单）列为二期**。
...
- ⚠️ **场景判定依赖业务方确认**（PRD 未决问题）：平台「境外」指跨境收单(A) 还是收款/结汇(B)。本研究默认 B，若实际是 A，境外通道选型与押金方案需重做（见 §5/§6）。
